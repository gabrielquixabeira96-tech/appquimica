    ...
    <div className="card overflow-hidden">
      <div className="flex items-start gap-3 border-b border-line p-5" style={{ borderTop: `3px solid ${eixo.cor}` }}>
        <div>
          ...
          <h3 className="font-display text-xl font-bold">{eixo.titulo}</h3>
      ...
      <ul className="divide-y divide-line">
        {eixo.temas.map((tema) => (
          <li key={tema.slug}>
            <Link href={`/temas/${tema.slug}`} className="group flex items-center gap-3 px-5 py-3 hover:bg-surface2">
              <span className="font-mono text-xs text-muted">{tema.id}</span>
              <span className="flex-1 text-sm font-medium">{tema.titulo}</span>
              {tema.prioridade === 'critica' && <span className="chip text-err">cai muito</span>}
